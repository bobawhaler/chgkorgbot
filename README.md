...
*   **Настройка чата:**
    *   `/settimezone <часовой_пояс>`: Настраивает часовой пояс для корректного отображдения времени турниров.
...
2.  Бот принимает запрос и мгновенно отвечает Telegram (HTTP 200 OK), чтобы избежать повторных запросов.
3.  Команда ставится в очередь **Google Cloud Tasks** для асинхронной обработки.
4.  Бот обрабатывает команду в фоновом режиме: обращается к API [Турнирного сайта сообщества знатоков](https://rating.chgk.info), проводит расчеты и отправляет результат в чат.
5.  По списку можно создать опрос со сроком окончания. Результат подводится автоматически по истечении срока. В случае нескольких победителей, бот выбирает случайного из них.

## Запуск и настройка (для разработчиков)

Проект написан на Python с использованием Flask и предназначен для развертывания на Google App Engine. В качестве базы данных используется Google Cloud Datastore. Для асинхронной обработки задач используется сервис Google Cloud Tasks.
