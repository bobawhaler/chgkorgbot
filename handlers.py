    ...
    if is_private:
        ...
        if register_teams:
            sections.append(
                ...
                "• <code>/unregister</code> (как reply) — отменить регистрацию команды\n"
...
def handle_export_roster(chat_id, thread_id=None):
    ...
    for reg in valid_regs:
        ...
        if teams and any(t.get("roster") or t.get("roster_submitted") for t in teams):
            ...
            tourn_name_safe = "".join(c for c in tourn_name if c.isalnum() or c in (" ", "_", "-")).strip().replace(" ", "_")
            filename = f"roster_{sync_req_id}_{tourn_name_safe}.csv"
            caption = f"📊 CSV со сданными составами турнира «{tourn_name}»"
            telegram_api.send_document(chat_id, csv_content, filename, caption=caption, message_thread_id=thread_id)
        ...
        if len(parts) > 1 and parts[1].strip():
            ...
            if arg.isdigit():
                ...
                if not found:
                    ...
                    return True
                lines = [f"Результаты поиска игроков по запросу «{arg}»:"]
                for idx, p in enumerate(found[:8], 1):
                    p_name = f"{p.get('name', '')} {p.get('surname', '')}".strip()
                    t_str = f" ({p['town']})" if p.get("town") else ""
                    lines.append(f"{idx}. [ID <code>{p['id']}</code>] <b>{p_name}</b>{t_str}")
                lines.append("")
                lines.append("Для привязки отправьте команду:\n<code>/setmyid <ID></code>")
                telegram_api.send_message(chat_id, None, "\n".join(lines), formatted=True)
                return True
            ...
            if cmd in ("/tourns", "/rtourns"):
                ...
                if len(inp) > 1:
                    ...
                    role_labels = {
                        ...
                        "rep": "🏛 Представитель (все команды турниров площадки)",
                        "player": "👥 Команда (только своя команда)",
                    ...
                    msg = (
                        ...
                        f"• <code>/role rep</code> — 🏛 Представитель (видит все команды площадки)\n"
                        f"• <code>/role player</code> — 👥 Команда (видит только свою команду)\n"
